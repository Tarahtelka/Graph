...
namespace GraphApp.BusinessLogic
{
    public class MathGraph : IMathGraph
    {
        ...
        public bool СompletenessGraff(Graph graph)
        {
            ...
            {
                throw new ArgumentException("Введите граф");
            }
            if ((graph.Vertices.Count * (graph.Vertices.Count - 1)) / 2 == graph.Edges.Count)
                return true; 
            else
                return false;
        }
        ...
        public Graph LoadGraphFromAdjacencyMatrix(string path)
        {
            ...
            var graph = this.CreateGraphWithVertices(vertexCount);
            ...
            using (var writer = new StreamWriter(File.Open(path, FileMode.OpenOrCreate)))
            {
                ...
                for (var i = 0; i < vertexCount; i++)
                {
                    for (var j = 0; j < vertexCount; j++)
                    {
                        ...
                        if (edgeForRemove != null)
                        {
                            writer.Write($"{edgeForRemove.Weight} ");
                        }
                        else
                        {
                            writer.Write("0 ");
        ...
        {
            var graph = this.CreateEmptyGraph();
